...
def of_tracker(v, file_name):
    ...
    x = c + w / 2
    y = r + h / 2
    p0 = np.array([[[x, y]], [[x + w / 5, y + h / 5]], [[x - w / 5, y - h / 5]],
                   [[x + h / 10, y + h / 10]], [[x - h / 10, y - h / 10]], [[
                       x - w / 5, y + h / 5
                   ]], [[x, y + h / 5]], [[x - w / 5, y]]], np.float32)
...
def kalman_tracker(v, file_name):
    ...
    kalman.processNoiseCov = 1e-5 * np.eye(
    ...
    while (1):
        ...
        img2 = cv2.rectangle(frame, (int(posterior[0]), int(posterior[1])),
                             (int(posterior[0]), int(posterior[1])), (255, 0,
                                                                      0), 3)
...
def particle_tracker(v, file_name):
    ...
    while (1):
        ...
        particles = particles.clip(
            np.zeros(2), np.array((im_w - 10, im_h - 10)) - 1).astype(int)
        ...
        for particle in particles:
            img2 = cv2.rectangle(frame, (particle[0], particle[1]),
                                 (particle[0], particle[1]), (255, 255, 0), 3)
